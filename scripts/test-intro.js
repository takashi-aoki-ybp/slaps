...
const tests = [
  ...
  ['minimum hold then slow fade after advancing playback', () => { const h=harness();h.play();h.advance(2500);assert.equal(h.fading,false);h.advance(200);assert.equal(h.fading,true);h.advance(1500);assert.equal(h.button.hidden,true);h.advance(100);assert.equal(h.button.hidden,false);assert.equal(h.timers.size,0); }],
  ['late playback starts fade only after stable progress', () => { const h=harness();h.advance(6000);h.play();h.advance(700);assert.equal(h.fading,false);h.advance(500);assert.equal(h.fading,true);h.advance(1700);assert.equal(h.intro.removed,true); }],
  ...
  ['buffering during fade cancels reveal', () => { const h=harness();h.play();h.advance(2800);assert.equal(h.fading,true);h.buffer();h.advance(3000);assert.equal(h.fading,false);assert.equal(h.intro.removed,false);h.play();h.advance(3000);assert.equal(h.intro.removed,true); }],
